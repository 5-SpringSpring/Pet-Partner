...
class LoveController (private val loveService: LoveService) {
    ...
    //좋아요 생성
    @PostMapping
    fun createLove(@RequestBody loveRequest: LoveRequest): ResponseEntity<LoveResponse> {
        return ResponseEntity.status(HttpStatus.CREATED).body(loveService.createLove(loveRequest))
    }
